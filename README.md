...
**If port 8545 is already taken**, set `LOCAL_RPC_PORT` on each of the three contracts commands (1 to 3 above).
...
LOCAL_RPC_PORT=8548 npm run relayer --workspace @rushood/contracts
```
...
The web app (step 4) is a separate workspace and does not read `LOCAL_RPC_PORT`; point it at a moved node with `NEXT_PUBLIC_RPC_URL`.
