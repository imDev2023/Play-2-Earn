...
### Minimum-bet floor
- `minBet = per-play cost ÷ edge`. Per-play cost on 4663 (an L2, in-house RNG so **no oracle fee**) is **cents** ⇒ **minBet well under $1** (target ~$0.25–0.50-equivalent; **[spec-resolved]** finalized at deploy against live gas).
