...
import { RELAYER_NETWORKS } from "../scripts/service/networks";
...
describe("relayer service config (#39)", () => {
  ...
  it("rejects a malformed game address", () => {
    expect(() =>
      loadRelayerConfig({ ...VALID_ENV, RELAYER_GAME_ADDRESS: "not-an-address" }),
    ).to.throw(/RELAYER_GAME_ADDRESS/);
  });
  ...
  it("lets an explicit environment seed win over the credential", () => {
    const env = withCredentialSeed(
      { ...VALID_ENV, CREDENTIALS_DIRECTORY: "/run/credentials" },
      () => "from-file",
    );
  ...
  it("still fails naming RELAYER_SEED when the credential cannot be read", () => {
    const { RELAYER_SEED: _omitted, ...withoutSeed } = VALID_ENV;
    const env = withCredentialSeed(
      { ...withoutSeed, CREDENTIALS_DIRECTORY: "/run/credentials" },
      () => {
...
describe("relayer network book (#61)", () => {
  const NETWORK_ENV = {
    RELAYER_NETWORK: "robinhoodTestnet",
    RELAYER_PRIVATE_KEY: VALID_ENV.RELAYER_PRIVATE_KEY,
    RELAYER_SEED: VALID_ENV.RELAYER_SEED,
  };

  /**
   * The committed entry is compared against the published deployment record rather
   * than against a copy of the same literals, because a test whose both sides come
   * from the test asserts nothing. The record is generated by `verify-and-publish.ts`
   * from the chain itself, which makes it the closest committed artefact to the
   * truth. The entry's name and chain id are parsed out of the record's heading for
   * the same reason - a literal 46630 here would only restate the book. The RPC URL
   * is the one field this cannot pin, because the record does not carry one.
   */
  it("keeps the testnet entry identical to the published deployment record", () => {
    const record = readFileSync(
      join(__dirname, "..", "..", "..", "docs", "deployments", "robinhoodTestnet.md"),
      "utf8",
    );
    const heading = record.match(/^# RUSHOOD deployment - (\w+) \(chain (\d+)\)/);
    expect(heading, "deployment record has no parseable heading").to.not.equal(null);
    const entry = RELAYER_NETWORKS[heading![1]];
    expect(entry, `the network book has no entry named ${heading![1]}`).to.not.equal(undefined);
    expect(entry.chainId).to.equal(Number(heading![2]));
    const row = record.match(/^\|\s*`RushoodGame`\s*\|\s*\[`(0x[0-9a-fA-F]{40})`\]/m);
    expect(row, "deployment record has no RushoodGame row").to.not.equal(null);
    expect(entry.gameAddress).to.equal(row![1]);
  });

  it("supplies the RPC URL, game address and chain expectation from the committed entry", () => {
    const config = loadRelayerConfig(NETWORK_ENV);
    expect(config.rpcUrl).to.equal(RELAYER_NETWORKS.robinhoodTestnet.rpcUrl);
    expect(config.gameAddress).to.equal(RELAYER_NETWORKS.robinhoodTestnet.gameAddress);
    expect(config.expectedChainId).to.equal(46630);
  });

  /**
   * The runbook's mid-incident move is switching RELAYER_RPC_URL to a fallback
   * provider - a different endpoint for the same chain. The URL override must win,
   * and the chain expectation must survive it, because a hurried paste is exactly
   * when a wrong-chain URL is most likely.
   */
  it("lets explicit variables win over the entry, without dropping the chain expectation", () => {
    const config = loadRelayerConfig({
      ...NETWORK_ENV,
      RELAYER_RPC_URL: "https://fallback.example/rpc",
      RELAYER_GAME_ADDRESS: VALID_ENV.RELAYER_GAME_ADDRESS,
    });
    expect(config.rpcUrl).to.equal("https://fallback.example/rpc");
    expect(config.gameAddress).to.equal(VALID_ENV.RELAYER_GAME_ADDRESS);
    expect(config.expectedChainId).to.equal(46630);
  });

  it("refuses an unknown network name, listing the known ones", () => {
    expect(() => loadRelayerConfig({ ...NETWORK_ENV, RELAYER_NETWORK: "robinhood" })).to.throw(
      /RELAYER_NETWORK "robinhood" is not in the committed network book \(known: robinhoodTestnet\)/,
    );
  });

  it("carries no chain expectation when the configuration is explicit variables alone", () => {
    expect(loadRelayerConfig(VALID_ENV).expectedChainId).to.equal(undefined);
  });

  it("passes boot when the endpoint answers as the expected chain", () => {
    const config = loadRelayerConfig(NETWORK_ENV);
    expect(() => assertExpectedChain(config, 46630)).to.not.throw();
  });

  it("refuses boot when the endpoint answers as another chain, naming both", () => {
    const config = loadRelayerConfig(NETWORK_ENV);
    expect(() => assertExpectedChain(config, 31337)).to.throw(
      /expects chain 46630.*answered as chain 31337/,
    );
  });

  it("asserts nothing when no expectation exists", () => {
    expect(() => assertExpectedChain(loadRelayerConfig(VALID_ENV), 31337)).to.not.throw();
  });

  it("names the expected chain in the boot description", () => {
    const line = describeConfig(
      loadRelayerConfig(NETWORK_ENV),
      "0x0000000000000000000000000000000000000009",
    );
    expect(line).to.include("chain 46630");
  });
});
